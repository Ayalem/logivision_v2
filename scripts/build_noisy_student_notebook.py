...
2. We collect tons of **unlabeled** warehouse footage every day. That
   data is wasted unless we can leverage it. Noisy Student is the
   standard self-training recipe (Xie et al. 2020, *Self-training
   with Noisy Student improves ImageNet classification*) — it
   propagates the teacher's knowledge to unlabeled data via
   pseudo-labels, then trains a (typically larger) student under
   noise to push past the teacher.
...
# Cell 5 — collect unlabeled warehouse video frames
code(
    """# 5. Collect UNLABELED warehouse frames. For this notebook we use the
#    Pexels warehouse videos shipped under datasets/raw/pexels_warehouse/
#    (15 short clips, total ~5 min of footage). In production this folder
#    grows as new RTSP camera dumps arrive — this cell scales automatically.
#
#    Frames are extracted at 1 fps to keep the pseudo-label set focused
#    on diverse scenes, not redundant consecutive frames.
import cv2, pathlib

UNLABELED_DIR = pathlib.Path('data/processed/unlabeled_warehouse/images')
UNLABELED_DIR.mkdir(parents=True, exist_ok=True)

SOURCE_VIDEOS = sorted(pathlib.Path('datasets/raw/pexels_warehouse').glob('*.mp4'))
print(f'source videos: {len(SOURCE_VIDEOS)}')
if not SOURCE_VIDEOS:
    print('NOTE: datasets/raw/pexels_warehouse is empty in a fresh clone (gitignored).')
    print('      For Colab, the clone is shallow so videos are missing.')
    print('      Falling back to using validation IMAGES from Kaggle as pseudo-input.')
    SOURCE_IMAGES = sorted((prep.OUT / 'images' / 'val').glob('*'))
else:
    SOURCE_IMAGES = []
...
for vid in SOURCE_VIDEOS:
    ...
    step = max(1, int(fps // EXTRACT_FPS))
    idx = 0
    while True:
        ok, frame = cap.read()
        if not ok: break
        if idx % step == 0:
            out = UNLABELED_DIR / f'{vid.stem}_f{idx:05d}.jpg'
            cv2.imwrite(str(out), frame)
            n_extracted += 1
        idx += 1
    cap.release()

# Fallback to Kaggle val images if no videos were available.
if not n_extracted and SOURCE_IMAGES:
    import shutil
    for p in SOURCE_IMAGES:
        shutil.copy(p, UNLABELED_DIR / p.name)
        n_extracted += 1

print(f'extracted {n_extracted} unlabeled frames to {UNLABELED_DIR}')
"""
)
