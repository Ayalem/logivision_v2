...
Ce document raconte **l'avancement concret** du projet : quels sprints ont été faits, dans quel ordre, quels problèmes ont été rencontrés, et comment ils ont été résolus. À lire en complément de `PROJECT_PLAN.md` (le plan théorique) et de `PROGRESS.md` (le suivi par tâche).
...
2. **MLflow port `5050` par défaut** (au lieu de `5000` que dit PROJECT_PLAN.md). Raison : macOS Control Center / AirPlay Receiver bind `:5000` par défaut, conflit impossible à résoudre sans désactiver AirPlay côté système. La variable d'env `MLFLOW_PORT` reste configurable.
...
4. **`dvc.yaml` à la racine du repo**, pas dans `ml/`. PROJECT_PLAN.md disait `ml/dvc.yaml` mais ses propres exemples de paths (`datasets/raw/videos`) sont relatifs à la racine. Mettre le fichier à la racine évite tout `..` ou path hack.
...
- **DVC `${VAR}` substitution** ne marche pas sans `vars:` au top de `dvc.yaml`. PROJECT_PLAN.md ne le précise pas ; ajout du bloc `vars: [ml/configs/data.yaml]`.

### 3.5 Workflow & itération

- **PROJECT_PLAN.md modifié hors-bande** par l'utilisateur pendant une session : ajout de sections "Workflow Opérationnel" et "Politique d'Autorisation". Plus un typo `1` puis `11` près d'un schéma ASCII. Règle §14 (modifier PROJECT_PLAN.md → PR dédiée) respectée via une PR `docs(plan): add operational workflow and authorization policy`. Le typo a été retiré dans la même PR.
...
```
logivision/
├── PROJECT_PLAN.md             ← plan théorique (1500+ lignes, autorité)
...
- **Frontend track** (non planifié dans PROJECT_PLAN.md, à patcher) : dashboard React/Vite/Tailwind.
