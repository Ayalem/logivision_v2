...
import { Lock, Mail, Eye, EyeOff, Warehouse, CheckCircle2, User, UserPlus, ArrowLeft } from 'lucide-react'
...
export function LoginPage() {
  ...
  const [registerData, setRegisterData] = useState({ name: '', email: '', password: '', role: 'worker' as 'admin' | 'worker' })
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/register', {
        ...
        body: JSON.stringify({
          ...registerData,
          role: registerData.role === 'admin' ? 'admin' : 'operator'
        })
      })
            ...
            <form onSubmit={handleRegister} className="space-y-6">
              {registerSuccess ? (
                <div className="py-8 text-center space-y-4">
                  ...
                  <h3 className="text-xl font-bold text-white">Compte Créé !</h3>
                  <p className="text-sm text-muted-foreground">Votre compte a été enregistré avec succès dans la base de données.</p>
                </div>
              ) : (
                <>
                  ...
                  <div className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        className="w-full px-4 py-2.5 rounded-lg bg-white/5 border border-white/10 text-sm text-white focus:outline-none focus:border-electric/50"
                      />
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        placeholder="jean@logivision.ai"
                        className="w-full px-4 py-2.5 rounded-lg bg-white/5 border border-white/10 text-sm text-white focus:outline-none focus:border-electric/50"
                      />
                    </div>
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Rôle</label>
                      <div className="grid grid-cols-2 gap-3">
                        <button
                          ...
                          className={cn(
                            ...
                            registerData.role === 'admin' ? 'bg-electric/20 border-electric text-electric' : 'bg-white/5 border-white/10 text-muted-foreground'
                          )}
                        >
                          Administrateur
                        </button>
                        <button
                          type="button"
                          onClick={() => setRegisterData({...registerData, role: 'worker'})}
                          className={cn(
                            'py-2.5 rounded-lg font-bold text-xs transition-all border',
                            registerData.role === 'worker' ? 'bg-electric/20 border-electric text-electric' : 'bg-white/5 border-white/10 text-muted-foreground'
                          )}
                        >
                          Opérateur
                        </button>
                  ...
                  <button
                    type="submit"
                    disabled={isLoading}
                    className="w-full py-3 rounded-xl bg-electric text-white font-bold text-sm shadow-lg shadow-electric/20 hover:bg-electric/90 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                  >
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2.5 rounded-lg bg-white/5 border border-white/10 text-sm text-white focus:outline-none focus:border-electric/50"
                  />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="••••••••"
                    className="w-full pl-10 pr-10 py-2.5 rounded-lg bg-white/5 border border-white/10 text-sm text-white focus:outline-none focus:border-electric/50"
                  />
                  <button
                    type="button"
